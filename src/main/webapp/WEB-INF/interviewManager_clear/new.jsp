...
$(document).ready(function () {
    ...
    $(document).on("click", ".save-button", function (event) {
        ...
        if (JSON.stringify(originalData) !== JSON.stringify(currentData)) {
            ...
            $.ajax({
                url: "<%= request.getContextPath() %>/interviewResult/clear",
                method: "POST",
                data: {
                    action: "update",
    ...
    $(".arrow-button").click(function () {
        if (!$(".result-management").length) {
            $(".container").append(`
                <div class="result-management">
                    <div class="form-container">
                        <form method="post" action="<%= request.getContextPath() %>/interviewResult/clear">
                            <input type="hidden" id="interviewId" name="interviewId" value="" /> <!-- hidden 필드 -->
                            <label for="title">면담 주제</label>
                            <input type="text" id="title" name="title" disabled required>
                            
                            <label for="summary">면담 요약</label>
                            <textarea id="summary" name="summary" disabled required></textarea>
                            ...
                            <textarea id="feedback" name="feedback" disabled required></textarea>
...
<body>
	<div class="container">
		<!-- 왼쪽 사이드 메뉴 -->
		<aside class="sidebar">
			<button class="menu-button">면담 신청</button>
			<button class="menu-button selected">면담 관리</button>
		</aside>
		...
		<main class="main-content">
			...
			<div class="interview-grid">
				...
				%>
				<div class="no-data">완료된 면담이 없습니다.</div>
				<%
				}
				%>
			</div>
